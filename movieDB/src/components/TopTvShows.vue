...
<template>
    <section class="">
        ...
        <h1 class="text-white text-center text-2xl xs:text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-semibold mb-14">
            Top 5 Popular TV Shows</h1>
        <div class="grid gap-1 px-4 sm:grid-cols-2 xs:px-8 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                <RouterLink class="mx-auto my-2 grid border py-4 px-3 text-center text-white bg-gradient-to-r from-blue-950 to-indigo-950 transition-all duration-200 hover:scale-105 active:scale-95 hover:bg-gradient-to-r hover:from-blue-900 hover:to-indigo-900 active:bg-gradient-to-r active:from-green-800 active:to-lime-700" v-bind:to="'/tv-show/'+tv.id" v-for="tv in state.tv.slice(0,5)">
                    ...
                    <p class="text-sm sm:text-md md:text-lg py-1 font-semibold">{{ tv.name }}</p>
