...
onMounted(async () => {
    ...
    try {
        ...
        console.log(state.movie.length);
    }
    catch (error) {
        console.error(`Error Occured While Fetching Results: ${error}`);
    } finally {
...
<template>
    <section class="grid lg:grid-cols-2 lg:my-2 relative">
        <img :src="'https://image.tmdb.org/t/p/original/' + state.movie.backdrop_path" alt=""
            class="absolute -top-1 inset-0 w-screen h-screen object-cover opacity-5">
        <div>
            <img :src="state.movie.poster_path ? 'https://image.tmdb.org/t/p/w500' + state.movie.poster_path : '../../assets/image/no-image.png'"
                alt="" class="w-60 xs:w-72 sm:w-80 xl:w-96 my-5 mx-auto">
        </div>
        ...
        <div class="grid lg:col-span-2">
            ...
            <p class="ml-2 my-2 lg:my-4 xl:ml-4 xs:text-lg "><span class="text-amber-400">Budget:</span> ${{
                state.movie.budget }}</p>
            <hr class="opacity-35 mx-1">
            <p class="ml-2 my-2 lg:my-4 xl:ml-4 xs:text-lg "><span class="text-amber-400">Revenue:</span> ${{
                state.movie.revenue }}
            </p>
            <hr class="opacity-35 mx-1">
            <p class="ml-2 my-2 lg:my-4 xl:ml-4 xs:text-lg "><span class="text-amber-400">Runtime:</span> {{
                state.movie.runtime }} Minutes
            </p>
            <hr class="opacity-35 mx-1">
            <p class="ml-2 my-2 lg:my-4 xl:ml-4 xs:text-lg "><span class="text-amber-400">Status:</span> {{
                state.movie.status }}</p>
            <hr class="opacity-35 mx-1">
